...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const baseMovieMap = new Map(seedMovies.map(m => [m.id, m]));
      const savedKeys = new Set(savedMovies.map(m => m.id));

      const mergedSavedMovies = savedMovies.map(savedMovie => {
        const baseMovie = baseMovieMap.get(savedMovie.id);
        if (!baseMovie) return savedMovie;

        return {
          ...baseMovie,
          ...savedMovie,
          dateAdded: Math.max(getDateMs(baseMovie.dateAdded), getDateMs(savedMovie.dateAdded)),
        };
      });

      const missingSeedMovies = seedMovies.filter(m => !savedKeys.has(m.id));
      setMovies([...missingSeedMovies, ...mergedSavedMovies]);
    })().catch((err) => {
  ...
  const handlePosterFileChange = async (file: File | undefined) => {
    ...
    setPosterUploadError('');
    try {
      const posterUrl = await resizePosterImage(file);
  ...
  const handleUsePoster = () => {
    if (!selectedMovie || !pendingPosterUrl) return;
    ...
    void (async () => {
      try {
        const { posterStoragePath, signedPosterUrl } = await uploadPosterDataUrlAndSign(
          supabase,
          uid,
          selectedMovie.id,
          pendingPosterUrl
  ...
  const handleAddMovie = async () => {
    ...
    try {
      ...
      const uid = supabaseUidRef.current;
      if (supabase && uid) {
        void upsertSavedMovieFromUI(supabase, uid, newMovie).catch((err) => {
  ...
  return (
    <div className="w-screen h-screen bg-[#000] antialiased font-sans overflow-hidden">
      ...
      {/* Sidebar */}
	      <aside className={`${isSidebarOpen ? 'w-64 border-r' : 'w-0 border-r-0'} flex h-full min-h-0 flex-col border-white/5 sidebar-gradient transition-all duration-300 ease-in-out overflow-hidden flex-shrink-0 relative z-10`}>
          ...
          >
              ...
              {modalMode === 'trailer' ? (
                ...
                <>
                  ...
                  />
                  <div className="absolute left-0 right-0 bottom-0 z-20 flex flex-col gap-3 bg-gradient-to-t from-black/85 via-black/55 to-transparent p-4">
                    {posterUploadError && (
                      <div className="self-center rounded-md border border-red-400/20 bg-red-500/15 px-3 py-2 text-[12px] font-medium text-red-200">
                        {posterUploadError}
                      </div>
                    )}
                    <div className="flex items-center justify-center gap-3">
                      <label className="inline-flex cursor-pointer items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-[12px] font-bold uppercase tracking-widest text-white/80 transition-colors hover:bg-white hover:text-black">
                        <Upload size={14} />
                        Upload Local Poster
                        <input
                          type="file"
                      ...
                      {pendingPosterUrl && (
                        <>
                          <button
                            ...
                            className="rounded-full bg-white px-5 py-2 text-[12px] font-bold uppercase tracking-widest text-black transition-colors hover:bg-white/85"
                          >
                            Use Poster
                          </button>
                          <button
                            ...
                            className="rounded-full bg-white/10 px-4 py-2 text-[12px] font-bold uppercase tracking-widest text-white/60 transition-colors hover:bg-white/15 hover:text-white"
                          >
                            Cancel
                          </button>
                        </>
